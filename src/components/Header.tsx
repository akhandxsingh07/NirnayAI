...
import { Menu, Mic, HelpCircle, Play, MapPin } from 'lucide-react';
    ...
    <header className="sticky top-0 z-30 border-b border-[#D9B99B]/40 bg-[#FAF7F3]/95 px-4 py-3 backdrop-blur-md sm:px-8">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <div className="hidden items-center gap-2 rounded-xl border border-[#D9B99B]/60 bg-white px-2.5 py-1.5 shadow-2xs md:flex">
            ...
            <select
              ...
              className="max-w-36 bg-transparent text-xs font-bold text-[#4A2F24] outline-none"
            >
          ...
          <button
            ...
            title={`${demoT('tryDemo', language)} — ${selectedCity.label}`}
          ...
          >
            ...
            <span className="hidden sm:inline">{demoT('tryDemo', language)} · {selectedCity.label}</span>
            <span className="sm:hidden">{demoT('demo', language)}</span>
          </button>
      ...
      <div className="mx-auto mt-2 flex max-w-7xl items-center gap-2 md:hidden">
        <MapPin className="h-3.5 w-3.5 text-[#B9825B]" />
        <select
          value={selectedDemoCity}
          onChange={(e) => onDemoCityChange(e.target.value as DemoCityId)}
          aria-label={demoT('selectCity', language)}
          className="w-full rounded-xl border border-[#D9B99B]/60 bg-white px-3 py-2 text-xs font-bold text-[#4A2F24] outline-none"
        >
          {DEMO_CITIES.map((city) => (
            <option key={city.id} value={city.id}>{city.label}</option>
          ))}
